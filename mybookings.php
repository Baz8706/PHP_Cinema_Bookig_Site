...
$results=$conn->query("SELECT m.ID, m.ntickets, m.showingID, f.film, f.poster, f.date, f.time, f.screen  FROM mybookings m, filmshowings f WHERE username = '$_SESSION[gatekeeper]' AND f.ID = m.showingID ;");
while($row=$results->fetch())
{
	echo "<form method='get' action='removebooking.php'><input type='hidden' value='$row[ID]' name='bID'</input><input type='hidden' value='$row[ntickets]' name='nt'</input><input type='hidden' value='$row[showingID]' name='sID'</input></form>";
	...
	echo "<td><a href='removebooking.php?bookingID=$row[ID]&ntID=$row[ntickets]&sID=$row[showingID]'>Cancel Booking</a></td>";
	echo "</tr>";
	echo "</table>";
